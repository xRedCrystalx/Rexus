...
class ReactionFilter:
    def __init__(self) -> None:
        ...
        self.db: dict[int, dict[str, discord.Message | dict[int, int] | list[str]]] = {}
        
    async def check_reaction(self, guild_db: dict[str, typing.Any], payload: discord.RawReactionActionEvent, **OVERFLOW) -> None:
        try:
            if guild_db["reaction"]["status"] and str(payload.emoji) in self.blacklist and payload.member and payload.guild_id:
                if not self.db.get(payload.message_id):
                    channel: discord.TextChannel = payload.member.guild.get_channel(payload.channel_id)
                    self.db[payload.message_id] = {"msg" : None, "users" : {}, "emojis" : [], "channel" : channel}
                ...
                if (log_channnel_id := guild_db["reaction"]["log_channel"]):                    
                    ...
                    embed.set_footer(text=f"Reaction has been removed.")
                ...
                if self.db[payload.message_id]["users"].get(payload.user_id):
                    self.db[payload.message_id]["users"][payload.user_id] += 1
                else:
                    self.db[payload.message_id]["users"][payload.user_id] = 1
    ...
    async def update(self, message_id: int) -> None:
        try:
            message: discord.Message = self.db[message_id]["msg"]
            if not message:
                channel: discord.TextChannel = self.db[message_id]["channel"]
                if channel:
                    message: discord.Message = await channel.fetch_message(message_id)
                    self.db[message_id]["msg"] = message
                else:
                    self.db.pop(message_id)
            ...
            for user_id, counter in self.db[message_id]["users"].copy().items():
                if counter >= 5:
                    if role_id := guild_db["reaction"]["role"]:
                        member: discord.Member | None = message.guild.get_member(user_id)
