...
class MessageListeners(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        if message.guild and message.author.id != self.bot.user.id: # remove this in future
            shared.loop.create_task(shared.queue.add_to_queue(e="on_message", message=message, guild_id=message.guild.id))
    ...
    async def on_raw_message_edit(self, payload: discord.RawMessageUpdateEvent) -> None:
        ...
        if payload.guild_id: # remove this in future
            shared.loop.create_task(shared.queue.add_to_queue(e="on_raw_message_edit", guild_id=payload.guild_id, payload=payload))

    @commands.Cog.listener()
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent) -> None:
        """If the message is found in the message cache, it can be accessed via `RawMessageDeleteEvent.cached_message`."""
        if payload.guild_id: # remove this in future
            shared.loop.create_task(shared.queue.add_to_queue(e="on_raw_message_delete", guild_id=payload.guild_id, payload=payload))
