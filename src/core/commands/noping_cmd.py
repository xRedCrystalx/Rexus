...
class NoPing(commands.GroupCog, name="noping"):
    ...
    @app_commands.guilds(discord.Object(id=938804743122346074))
    ...
    @app_commands.command(name="info", description="General/Info command of NoPing")
    async def info(self, interaction: discord.Interaction, cmd: app_commands.Choice[str], args: str = None) -> None:
        guild_db: dict[str, typing.Any] = self.shared.db.load_data(interaction.guild.id)
        bot_config: dict[str, typing.Any] = self.shared.db.load_data()
    ...
    async def restart(self, interaction: discord.Interaction, args: str | None) -> None:
