    ...
    async def noping(self, interaction: discord.Interaction, cmd: app_commands.Choice[str], args: str = None) -> None:
        ...
        if cmd.value == "help":
            ...
            embed.add_field(name="Quick Start", value=f"__Due to security standards, only **guild owner** ({interaction.guild.owner.display_name}) can configure bot.__\n \
                            Owner can **grant/rewoke** configuration permissions to **everyone** with `Administrator guild permission` using **</admin_editing:0>** slash command.\n\n \
                            By default, Bot is **disabled** and will have to be **enabled** in `General settings` using **</config:1234484595445993542>** slash command.\n \
            ...
            embed.add_field(name="─────────────────────────────────────────────────────────────", value="**Navigation:**\n \
                            ...
                            **General:**\n \
                            - You have **1 hour** to configure each time you execute **</config:1234484595445993542>** command. You will be notified if that time is reached.\n- Please **read** instructions, they are important.\n- Each plugin has its own `switch`, meaning you can disable them anytime for whatever reason.\
                            \nFor any questions and concerns, feel free to create **feedback or bug report** using **</noping:1234489037608456192>** command, or join the [Support Server](https://discord.gg/gzx3kxu68x)", inline=False)
