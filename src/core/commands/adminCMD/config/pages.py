...
class HelpPages:
    ...
    cmd: Embed = create_base_embed("Commands")
    ...
    alt: Embed = create_base_embed("Alt Detection", description="Alt detection is a plugin that detects new joined members and checks their account creation date.")
    ...
    imper: Embed = create_base_embed("Impersonator Detection", description="Impersonator Detection is a plugin that detects new joined members and member profile updates. Bot checks their display and global name for any known youtuber/company/celebrity names etc. Real accounts are ignored.")
    ...
    ai: Embed = create_base_embed("Artificial Intelligence", description="Artificial Inteligence (computer that understands human text and responds to it). Talk to NoPing!\n**Prefix:** `> ` (space)")

    automod: Embed = create_base_embed("Automod Responses", description="Automod Responses is a plugin that sends custom message under Automod's trigger message. Normally used for automatically creating warn/ban/kick commands for other bots, for example Dyno, MEE6, Probot...\nTo use placeholders, use `{...}` *Example: ?warn {user.id} Spamming*")
    ...
    link: Embed = create_base_embed("Link Protection", description="Link Protection is a plugin that detects new messages and tries to find link matches. Plugin logs and deletes detected messages.")
    ...
    ping: Embed = create_base_embed("Ping Protection", description="Ping Protection is a plugin that detects new messages and tries to find ping matches. If rule was broken and user doesn't have `BypassRole`, warns, logs and calls staff (optional).")
    ...
    auto_delete: Embed = create_base_embed("Auto Delete", description="AutoDelete is a plugin that deletes new created messages after x seconds/minutes/hours (depending of configuration) for the channel.")
    ...
    auto_slowmode: Embed = create_base_embed("Auto Slowmode", description="AutoSlowmode is a plugin that records messages every 5 minutes and determinates how active channel is. Automatically sets slowmode to different values (seconds).")
    ...
    QOFTD: Embed = create_base_embed("Question of the Day", description="This plugin posts a random question every day at midnight UTC+2.")
    ...
    reaction: Embed = create_base_embed("Reaction Filter", description="Reaction filter is a plugin that looks for a specific reactions and removes them. If a member continuously tries to add those reactions, bot reaction bans that member (if the role is provided in the config).")
...
class ConfigPages:
    ...
    alt: Embed = create_base_embed("Alt Detection", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}\n<:text_c:1203423388320669716>┇{log_channel:id_format?option='channel'}")
    alt.set_thumbnail(url="https://i.ibb.co/R6WZm04/member.png")
    
    imper: Embed = create_base_embed("Impersonator Detection", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}\n<:text_c:1203423388320669716>┇{log_channel:id_format?option='channel'}")
    imper.set_thumbnail(url="https://i.ibb.co/R6WZm04/member.png")

    ai: Embed = create_base_embed("Artificial intelligence", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}")
    ...
    automod: Embed = create_base_embed("AutoMod Response", description="Under development.\nUse **</automod load_rules:1160680000970821645>** and **</automod create_response:1160680000970821645>** commands.")

    link: Embed = create_base_embed("Link Protection", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}\n<:text_c:1203423388320669716>┇{log_channel:id_format?option='channel'}")
    ...
    auto_delete: Embed = create_base_embed("Auto Delete", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}")
    ...
    auto_slowmode: Embed = create_base_embed("Auto Slowmode", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}")
    ...
    QOFTD: Embed = create_base_embed("Question of the Day", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}")
    ...
    reaction: Embed = create_base_embed("Reaction Filter", description="<:bot:1226153730836140054>┇{status:boolean_format?option='switch'&discord_format}\n<:text_c:1203423388320669716>┇{log_channel:id_format?option='channel'}\n<:role:1226153759722438707>┇{reactionBanRole:id_format?option='role'}")
    ...
    def create_embed(self, data: dict, blueprint_embed: discord.Embed | None, interaction: discord.Interaction, name: str) -> discord.Embed:
        ...
        if blueprint_embed:
            ...
            embed: Embed = create_base_embed(name, description=f"<:bot:1226153730836140054>┇`{self.formatter.boolean_format(data.get("status"), option="switch")}`\n<:text_c:1203423388320669716>┇{self.formatter.id_format(data.get("log_channel"), "channel")}")
        return embed
